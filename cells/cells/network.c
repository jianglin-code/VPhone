...
static int ret_set_errno(int ret, int err)
{
...
static int syserror_set(int ret, const char* msg, ...)
{
...
{
	call_cleaner(nlmsg_free) struct nlmsgs *answer = NULL, *nlmsg = NULL;
...
{
	call_cleaner(nlmsg_free) struct nlmsgs *nlmsg = NULL;
	struct nl_handler nlh = NL_HANDLER_INIT;
	call_cleaner(netlink_close) struct nl_handler *nlh_ptr = &nlh;
	int err;
	struct ifinfomsg *ifi;
