...
static void* ret_set_errno(void* ret, int err)
{
	ALOGE("err = %s", strerror(err));
	return ret;
}

static int ret_errno(int err)
{
	ALOGE("err = %s", strerror(err));
	return -err;
}

static size_t nlmsg_len(const struct nlmsgs *nlmsg)
{
	return nlmsg->nlmsghdr->nlmsg_len - NLMSG_HDRLEN;
}

void *nlmsg_data(struct nlmsgs *nlmsg)
{
	char *data;
...
static int nla_put(struct nlmsgs *nlmsg, int attr,
...
int nla_put_buffer(struct nlmsgs *nlmsg, int attr, const void *data, size_t size)
{
	return nla_put(nlmsg, attr, data, size);
}

int nla_put_string(struct nlmsgs *nlmsg, int attr, const char *string)
{
	return nla_put(nlmsg, attr, string, strlen(string) + 1);
}

int nla_put_u32(struct nlmsgs *nlmsg, int attr, int value)
{
	return nla_put(nlmsg, attr, &value, sizeof(value));
}

int nla_put_u16(struct nlmsgs *nlmsg, int attr, unsigned short value)
{
	return nla_put(nlmsg, attr, &value, 2);
}

int nla_put_attr(struct nlmsgs *nlmsg, int attr)
{
	return nla_put(nlmsg, attr, NULL, 0);
}

struct rtattr *nla_begin_nested(struct nlmsgs *nlmsg, int attr)
{
...
void nla_end_nested(struct nlmsgs *nlmsg, struct rtattr *attr)
{
	attr->rta_len = (void *)NLMSG_TAIL(nlmsg->nlmsghdr) - (void *)attr;
}

struct nlmsgs *nlmsg_alloc(size_t size)
{
	/*__do_free*/ struct nlmsgs *nlmsg = NULL;
	size_t len = NLMSG_HDRLEN + NLMSG_ALIGN(size);

	nlmsg = malloc(sizeof(struct nlmsgs));
	if (!nlmsg)
		return ret_set_errno(NULL, ENOMEM);
...
void *nlmsg_reserve(struct nlmsgs *nlmsg, size_t len)
{
	void *buf;
...
struct nlmsgs *nlmsg_alloc_reserve(size_t size)
{
	struct nlmsgs *nlmsg;
...
void nlmsg_free(struct nlmsgs *nlmsg)
{
	if (nlmsg) {
		free(nlmsg->nlmsghdr);
		free(nlmsg);
...
int netlink_rcv(struct nl_handler *handler, struct nlmsgs *answer)
{
...
extern int netlink_transaction(struct nl_handler *handler,
			       struct nlmsgs *request, struct nlmsgs *answer)
{
