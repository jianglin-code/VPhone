...
/*
 * struct nlmsgs : the netlink message structure. This message is to be used to
 ...
 */
extern int netlink_rcv(struct nl_handler *handler, struct nlmsgs *nlmsg);
extern int __netlink_recv(struct nl_handler *handler, struct nlmsghdr *nlmsg);
 ...
 * Returns 0 on success, < 0 otherwise
 */
extern int netlink_send(struct nl_handler *handler, struct nlmsgs *nlmsg);
extern int __netlink_send(struct nl_handler *handler, struct nlmsghdr *nlmsg);
 ...
 * Returns 0 on success, < 0 otherwise
 */
extern int netlink_transaction(struct nl_handler *handler, struct nlmsgs *request,
					struct nlmsgs *answer);
extern int __netlink_transaction(struct nl_handler *handler, struct nlmsghdr *request,
 ...
 * Returns 0 on success, < 0 otherwise
 */
extern int nla_put_string(struct nlmsgs *nlmsg, int attr, const char *string);
 ...
 * Returns 0 on success, < 0 otherwise
 */
extern int nla_put_buffer(struct nlmsgs *nlmsg, int attr, const void *data, size_t size);
 ...
 * Returns 0 on success, < 0 otherwise
 */
extern int nla_put_u32(struct nlmsgs *nlmsg, int attr, int value);
 ...
 * Returns 0 on success, < 0 otherwise
 */
extern int nla_put_u16(struct nlmsgs *nlmsg, int attr, unsigned short value);
 ...
 * Returns 0 on success, < 0 otherwise
 */
extern int nla_put_attr(struct nlmsgs *nlmsg, int attr);
 ...
 * to nla_end_nested.
 */
extern struct rtattr *nla_begin_nested(struct nlmsgs *nlmsg, int attr);
 ...
 * Returns the current
 */
extern void nla_end_nested(struct nlmsgs *nlmsg, struct rtattr *attr);
 ...
 */
extern struct nlmsgs *nlmsg_alloc(size_t size);
 ...
 * @size: the capacity of the payload to be allocated.
 */
extern struct nlmsgs *nlmsg_alloc_reserve(size_t size);
 ...
 */
extern void *nlmsg_reserve(struct nlmsgs *nlmsg, size_t len);
 ...
 */
extern void nlmsg_free(struct nlmsgs *nlmsg);
//define_cleanup_function(struct nlmsgs *, nlmsg_free);
 ...
 */
extern void *nlmsg_data(struct nlmsgs *nlmsg);
