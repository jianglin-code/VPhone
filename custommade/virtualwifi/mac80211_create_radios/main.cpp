...
int main(int argc, char* argv[]) {
    if (argc < 2) { return printUsage(stdout, 0); }
    ...
    int macPrefix;
    if (argc == 2) {
        char mac_prefix[PROPERTY_VALUE_MAX] = {0};
        property_get("ro.boot.cells_wifi_mac_prefix", mac_prefix, "");
        if(strcmp(mac_prefix, "") != 0){
            macPrefix = atoi(mac_prefix);
        }else{
            srand(time(0));
            macPrefix = rand() % 65536;
        }
    } else {
