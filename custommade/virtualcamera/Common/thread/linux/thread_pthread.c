...
CAPI int Thread_Run(RTPThread *t) {
    ...
    if (t && t->t == 0) {
        ...
        ret = pthread_attr_setschedpolicy(&(t->attr), policy);
        ALOGD("pthread_attr_setschedpolicy = %d", ret);
        ...
        ALOGD("pthread_attr_setschedparam = %d", ret);
        ret = pthread_create(&(t->t), &(t->attr), pthread_func, t);
        ALOGD("pthread_create = %d", ret);
    }
    return ret;
}
